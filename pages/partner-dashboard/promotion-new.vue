            ...
            <v-col cols="12" sm="6">
              ...
              <div class="d-flex align-center justify-space-between">
                ...
                <v-text-field
                  ...
                  @click:append="how_many_person++"
                  @click:prepend-inner="how_many_person--"
                ></v-text-field>
              </div>
              <div class="d-flex align-center justify-space-between">
                <div>
                ...
                <v-radio-group v-model="menu_details" mandatory row>
                  <v-radio label="Course Menu" value="12"></v-radio>
                  <v-radio label="Buffet" value="1"></v-radio>
              ...
              <v-row>
                <v-col cols="12" sm="6">
                  <v-text-field
                    v-model="discount_condition_more_than_person"
                    label="Enter Amount"
                    outlined
                    hint="NOK"
                    persistent-hint=""
                ...
                <v-col cols="12" sm="6">
                  <v-text-field
                    v-model="discount_condition_price"
                    label="Booked for"
                    outlined
                    hint="Persons or more"
                    persistent-hint=""
                  ></v-text-field>
                </v-col>
              </v-row>
              ...
              <v-item-group v-model="MealType" multiple mandatory>
                <v-row>
                  <v-col
                  ...
                  <v-col cols="12" sm="6">
                    ...
                    <div class="pt-3 d-flex other-info-box">
                      ...
                      <div>
                        ...
                        <div class="mb-5">
                          <span
                            v-for="(item, index) in previewMealType"
                            :key="item.id"
                          >
                            {{ item.name
                            }}{{
                              index == previewMealType.length - 1 ? "" : ", "
                            }}
                          </span>
                        </div>
                      </div>
...
export default {
  ...
  data() {
    return {
      ...
      discount_condition_more_than_person: "",
      notAvailable: [
        {
          id: 1,
          label: "M",
  ...
  async mounted() {
    ...
    try {
      await this.$axios
        .post("GetSearchValue", { ParentId: "97" })
        .then(response => {
      ...
      try {
        await this.$axios
          .post("Mobile/Partner/InsertPromotions", {
            ...
            food_type: this.FoodType.toString(),
            meal_type: this.MealType,
            // PostItem: this.PostItem,
            Quantity: this.Quantity,
            ExcludeTo: "",
            OrderName: this.OrderName,
            pickup_start_time: this.PickupStartTime,
            pickup_end_time: this.PickupEndTime,
            sat_pickup_start_time: this.SatStartTime,
            sat_pickup_end_time: this.SatEndTime,
            sun_pickup_start_time: this.SunStartTime,
            sun_pickup_end_time: this.SunEndTime,
            exclude_day_order: "1, 2, 3",
            exclude_dates: "1, 3, 5, 7",
            // userID: this.$store.state.userData.userID.toString(),
            offer_type: this.offerType.toString(),
            AccessToken: this.$store.state.userData.userAccessToken,
            // To Be added
            promotions_from_date: this.dates[0],
            promotions_to_date: this.dates[1],
            ExcludeDates: "-",
            ExcludeDayOrder: "-",
            how_many_person: this.how_many_person.toString(),
            discount_description: this.discount_description,
            picture: this.offerImageEncoded,
            title: this.offerType,
            menu_details: this.menu_details,
            discount_condition_price: this.discount_condition_price,
            discount_condition_more_than_person: this
              .discount_condition_more_than_person
          })
          .then(response => {
  ...
  computed: {
    ...
    previewMealType() {
      if (this.mealTypes.length > 0 && this.MealType.length > 0) {
        let previewArray = [];
        this.MealType.forEach(element => {
          let temp = this.mealTypes.find(obj => obj.id == element);
          previewArray.push(temp);
        });
