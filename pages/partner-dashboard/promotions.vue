  ...
  <div class="partner-dash">
    <v-container>
      <div class="list-head">
        ...
        <nuxt-link to="/partner-dashboard/promotion-new" class="post-offer">
          <v-icon>mdi-plus</v-icon>New Promotion
        </nuxt-link>
      </div>
    </v-container>
          ...
          >
            <div class="post-list">
              ...
              <div class="card-body-sec">
                <p class="type">{{ item.offer_type }}</p>
                <h4 @click="getDetails(item)">{{ item.offer_title }}</h4>
                <p class="listing-price">
                  <span class="nondis-price">{{
                    restuarantData.bankcurrencyid + " " + item.price
                  }}</span>
                  <span>{{
                    restuarantData.bankcurrencyid +
                      " " +
                      item.discount_condition_price
                  }}</span>
                </p>
                <div class="days">
                  <p class="active">M</p>
                ...
                <p>
                  ...
                  <span
                    >{{ item.promotions_from_date }} -
                    {{ item.promotions_to_date }}</span
                  >
                </p>
              </div>
            </div>
    ...
    >
      ...
      <v-card tile>
        <v-toolbar dark color="#003680">
          <v-toolbar-title>Promotion Details</v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-img
          :src="currentPostDataSmall.picture"
          height="300"
          class="grey darken-4"
        ></v-img>
        <v-container>
          <div class="d-flex justify-space-between">
            <div>
              ...
              <h2 class="font-weight-regular">
                {{ currentPostData.offer_title }}
              </h2>
            </div>
            <div class="text-right">
              <v-btn color="success" elevation="0">{{
                currentPostDataSmall.offer_type
              }}</v-btn>
              <h1 class="text-h1 blue-text">
                <span style="font-size: .7em; text-decoration: line-through;">{{
                  restuarantData.bankcurrencyid +
                    " " +
                    currentPostDataSmall.price
                }}</span>
                {{
                  restuarantData.bankcurrencyid +
                    " " +
                    currentPostDataSmall.discount_condition_price
                }}
              </h1>
            </div>
          </div>
              ...
              <v-col cols="12" sm="6">
                ...
                <div class="pt-3 d-flex other-info-box">
                  ...
                  <div>
                    ...
                    <div class="mb-5">
                      <span
                        v-for="(item, index) in currentMealType"
                        :key="item.id"
                      >
                        {{ item.name
                        }}{{ index == currentMealType.length - 1 ? "" : ", " }}
                      </span>
                    </div>
                  </div>
                  <div>
    ...
    async getDetails(PostOrder) {
      ...
      await this.$axios
        .post("mobile/partner/GetPromotion", {
          partner_id: this.$store.state.userData.userID.toString(),
          AccessToken: this.$store.state.userData.userAccessToken,
          promotion_id: PostOrder.id.toString()
        })
        .then(response => {
          // console.log(response.data);
          this.currentPostData = response.data.id[0];
        })
        .then(async () => {
        ...
        .then(async () => {
          ...
          let currentMealTypeArray = this.currentPostData.meal_type.split(",");

          let tempArray = [];
          currentMealTypeArray.forEach(element => {
            let temp = mealTypes.data.data.find(
              obj => obj.id == parseInt(element)
            );
            this.currentMealType.push(temp);
          });
        });
    },
